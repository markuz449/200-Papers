...
public class Tableau {
    ...
    public void addValue(Integer value) {
        ...
        Integer addLocation = addToRow(smallest, value);
        if (addLocation == null){
            return;
        } else {
            Cell addBelow = new Cell(addLocation);
            Cell smallCopy = smallest;
            while (addLocation != null){
                if (smallCopy.below == null){
                    smallCopy.below = addBelow;
                    addBelow.above = smallCopy;
                    return;
                } else {
                    smallCopy = smallCopy.below;
                    addLocation = addToRow(smallCopy, addLocation);
                    if (addLocation == null){
                    } else {
                        addBelow.value = addLocation;
                    }
                }
            }        
        }    
    ...
    protected Integer addToRow(Cell curr, int value) {
        ...
        Cell endCell = new Cell(value);
        endCell.left = curr;
        curr.right = endCell;
        if (curr.above != null){
            endCell.above = endCell.left.above.right;
            endCell.above.below = endCell;
        }
        return null;
    }
